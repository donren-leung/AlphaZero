...
class MCTS_Instance():
    ...
    def __init__(self, rollouts: int, state: TicTacToeState, player: int) -> None:
...
class Node():
    def __init__(self, parent: 'Node' | None, parent_action: int | None, value: int | None, state: TicTacToeState, player: int) -> None:
        assert (
    ...
    def ucb_score(self, parent_visits: float) -> float:
        ...
        return (-self.value_sum / self.visits +
                MCTS.EXPLORATION_PARAM * math.sqrt(math.log(parent_visits) / self.visits)
        )
    ...
    def print_children(self, depth: int=0, limit: int=2) -> None:
        if depth == 0:
